...
bullet([("ลด cloud cost รายเดือนอย่างวัดได้", True),
        (" — ปิด platform migration 2 ตัวจบในไตรมาสเดียว (Azure SQL Server → self-hosted PostgreSQL "
         ...
         "วัดจากบิล Azure จริง: ", False),
        ("ค่าฐานข้อมูลรายเดือนลดลง 44%", True),
        (" (ทั้งที่รับงานใหม่ของระบบสแกนเข้ามาในบิลเดียวกัน) และ ", False),
        ("Data Factory เหลือ 0% ตั้งแต่วันปลดระวาง", True),
        (" · เหตุผลที่ย้ายมาจากการวัด ไม่ใช่ความรู้สึก — batch เดิมติด IO 98% บนคลาวด์ ใช้ 2 ชม. "
         "เทียบกับ 20 นาทีบนเครื่องโลคอล และการเพิ่ม vCore ทดสอบแล้วว่าแพงขึ้นโดยไม่คุ้ม · ", False),
